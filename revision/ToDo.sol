...
contract toDoList{
    ...
    function deleteATask(uint _taskId)external {
         require(_taskId<Task[msg.sender].length,"Invalid Task Id ");
         tasks[msg.sender][_taskId]= tasks[msg.sender][tasks[msg.sender][_taskId].length-1];//// Move the last task into the deleted spot, then pop so that the task Id will get updated and then it will not be confused.
